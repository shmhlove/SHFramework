...
public class SQLiteQuery {
	...
	public double GetDouble( string field )
	{
        ...
        if (Sqlite3.SQLITE_INTEGER == columnTypes[i])
        {
            return (double)Sqlite3.sqlite3_column_int(vm, i);
        }
    ...
    {
        if (0 > iIndex || iIndex > columnTypes.Length)
            throw new Exception("SQLite wrong field Index (expecting String) : " + iIndex);
        
        if (Sqlite3.SQLITE_TEXT == columnTypes[iIndex])
        {
            return Sqlite3.sqlite3_column_text(vm, iIndex);
        }

        throw new Exception("SQLite wrong field type (expecting String) : " + iIndex);
    }

    public int GetInteger(int iIndex)
    {
        if (0 > iIndex || iIndex > columnTypes.Length)
            throw new Exception("SQLite wrong field Index (expecting Integer) : " + iIndex);

        if (Sqlite3.SQLITE_INTEGER == columnTypes[iIndex])
        {
            return Sqlite3.sqlite3_column_int(vm, iIndex);
        }

        throw new Exception("SQLite wrong field type (expecting Integer) : " + iIndex);
    }

    public double GetDouble(int iIndex)
    {
        if (0 > iIndex || iIndex > columnTypes.Length)
            throw new Exception("SQLite wrong field Index (expecting Double) : " + iIndex);

        if (Sqlite3.SQLITE_FLOAT == columnTypes[iIndex])
        {
            return Sqlite3.sqlite3_column_double(vm, iIndex);
        }
        if (Sqlite3.SQLITE_INTEGER == columnTypes[iIndex])
        {
            return (double)Sqlite3.sqlite3_column_int(vm, iIndex);
        }

        throw new Exception("SQLite wrong field type (expecting Double) : " + iIndex);
    }

    public byte[] GetBlob(int iIndex)
    {
        if (0 > iIndex || iIndex > columnTypes.Length)
            throw new Exception("SQLite wrong field Index (expecting byte[]) : " + iIndex);

        if (Sqlite3.SQLITE_BLOB == columnTypes[iIndex])
        {
            return Sqlite3.sqlite3_column_blob(vm, iIndex);
        }

        throw new Exception("SQLite wrong field type (expecting byte[]) : " + iIndex);
    }
